...
This tells that usually more cylinders mean lower mpg, but some 6-cylinder cars still achieve above 20mpg
...
because these two factors are related, a lighter car with the same horsepower will accelerate faster than a heavier one. so by having these two metrics make comparisons more specific
...
inner join includes only students that appear in both tables
left join keeps every student from the demographics table
the average grade shows students' average score for several courses
---
...
semi_join gets active students
first anti_join shows students with no grades
second anti_join catches grade records with no match.
