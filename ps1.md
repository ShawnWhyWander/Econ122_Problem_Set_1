...
6-cylinder cars with mpg above 20.

This tells that usually more cylinders mean lower mpg, but some
6-cylinder cars still achieve above 20mpg
...
because these two factors are related, a lighter car with the same
horsepower will accelerate faster than a heavier one. so by having these
two metrics make comparisons more specific
...
cars with more cylinders usually have lower mpg and higher horsepower.
manual transmission often yield higher mpg. —
...
``` r
product_sales_wide_growth <- product_sales_wide %>%
  mutate(
    Growth_Y2021 = (Year_2021 - Year_2020) / Year_2020 * 100,
    Growth_Y2022 = (Year_2022 - Year_2021) / Year_2021 * 100
  )

product_sales_wide_growth
```

    ## # A tibble: 3 × 6
    ##   product  Year_2020 Year_2021 Year_2022 Growth_Y2021 Growth_Y2022
...
product_sales_long_growth <- product_sales_long %>%
  ...
  group_by(product) %>%
  mutate(
    GrowthRate = (Sales - lag(Sales)) / lag(Sales) * 100
  ) %>%
  ungroup()
...
## The wide version needs a formula every year, the long version only uses one mutate, which is more concise.
...
inner_join_df <- students_demographics %>%
...
left_join_df <- students_demographics %>%
...
avg_grade_student <- left_join_df %>%
  group_by(student_id, student_name) %>%
  summarise(avg_grade = mean(grade_score, na.rm = TRUE), .groups = "drop")
...
inner join includes only students that appear in both tables left join
keeps every student from the demographics table the average grade shows
students’ average score for several courses —
...
students_no_grades <- students_demographics %>%
...
grades_withno_demographics <- student_grades %>%
  anti_join(students_demographics, by = "student_id")

grades_withno_demographics
```
...
semi_join gets active students first anti_join shows students with no
grades second anti_join catches grade records with no match.
